...
module.exports = async function () {
    ...
    if(!semver.satisfies(process.versions.node, '>=16.9.0')) {
        throw Error(`❌ Bishop requires Node.js version >=16.9.0. You are using v${process.versions.node}. Please upgrade to continue.`);
    } else {
        ...
        if(fs.existsSync(`./config/${e}`)) {
            ...
            Object.keys(moduleJson).forEach(function (prop) {
                if(moduleJson[prop].length == 0) {
                    throw Error(`❌ Missing required config value [${prop}] in ${e}. Please update or add this value.`)
                }
            ...
            if(requiredFields[e].length > 0) {
                throw Error(`❌ Missing required config value(s) [${requiredFields[e].join(', ')}] in ${e}. Please update or add this value.`)
            } else {
