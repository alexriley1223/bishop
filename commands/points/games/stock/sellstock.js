...
module.exports = {
    ...
    .addStringOption(option =>
      option.setName('symbol')
        .setDescription('Symbol of stock to sell')
        .setRequired(true))
    ...
    await axios.get(`https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=${symbol}&apikey=${alphaToken}`)
      ...
      .catch(function (error) {
		...
		await Stocks.findOne({ where:
			...
			if(!stock) {
				interaction.reply({ content: `Unable to retrieve stock entry. Please try again.`, ephemeral: true });
				return;
			}

			if(numShares > stock.shares) {
				interaction.reply({ content: `Trying to sell too many shares. Please try again.`, ephemeral: true });
				return;
			}

			var stockSellPrice = stockPrice * numShares;
			...
			interaction.client.channels.cache.get(gameUpdatesChannelId).send(`<@${userId}> sold ${numShares} shares of ${symbol} for ${stockSellPrice} points!`);
			interaction.reply({ content: `Stock(s) sold for ${stockSellPrice} points!`, ephemeral: true });
		})
		.catch(function(error) {
			interaction.reply({ content: `Unable to retrieve stock entry. Please try again.`, ephemeral: true });
			return;
