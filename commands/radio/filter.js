...
module.exports = {
	data: new SlashCommandBuilder()
        ...
        .addSubcommand(subcommand =>
            subcommand.setName('show').setDescription('Show all filters'))
        .addSubcommand(subcommand =>
            subcommand.setName('toggle').setDescription('Toggle an audio filter').addStringOption(option =>
                option.setName('name').setDescription('Name of the filter').setRequired(true).addChoices(
 	...
 	async execute(interaction) {
        ...
        switch (subCmd) {
            case "clear":
                if (!filters.length) {
                    return await interaction.reply({ content: `No filters are currently applied`, ephemeral: true });
                }
            ...
            case "toggle":
                ...
                return await interaction.reply({ content: `Audio filter ${filterName} has been applied to Bot playback.` });
                break;
