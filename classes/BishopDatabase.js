...
module.exports = class BishopDatabase extends Sequelize {
   ...
   */
	async getMigrations() {
		this.migrations = await this.models.migrations
			.findAll({ attributes: ['name'], raw: true })
			.then((migrationNames) => migrationNames.map((migrationName) => migrationName.name));
		this.batchNumber =
      (await this.models.migrations
      	.findOne({ limit: 1, order: [['createdAt', 'DESC']], raw: true })
      	.then((firstMigration) => (firstMigration?.batch ? firstMigration.batch + 1 : null))) ?? 1;
	}
};
