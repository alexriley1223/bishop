...
(async () => {
	try {
		const commandList = await rest.get(Routes.applicationGuildCommands(clientId, guildId));
		...
		commands.forEach(function(command) {
			try {
				...
				rest.delete(Routes.applicationGuildCommand(clientId, guildId, command)).then(function() {
