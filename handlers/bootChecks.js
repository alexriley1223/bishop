	...
	await axios
		...
		.then((res) => {
			...
			if (versions[0].name != `v${version}`) {
				log.warn(
					'BOOT',
					`Bot is not the latest version. ${(semver.gt(`v${version}`, versions[0].name)) ? `Beta version in use. Proceed with caution!` : `Please update! Yours: v${version} - Latest: ${versions[0].name}`}`,
				);
			}
		})
		.catch((err) => {
